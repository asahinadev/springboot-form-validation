...
@SpringBootTest
public class FromToByLocalDateTest
		extends BasedFromToTest<LocalDate> {
	...
	public static List<ParameterBase<FromToType<LocalDate>>> data() {
		...
		list.add(new ParameterBase<>(FromToType.<LocalDate>builder().from(c1).to(c1).build(), 0));
	...
	public FromToByLocalDateTest(ParameterBase<FromToType<LocalDate>> parameter) {
